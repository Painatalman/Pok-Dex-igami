...
- **Sentinels** (new, supersedes the first Ditto note) — Two roster entries mean "this photo is not a folded model", declared in a `SENTINELS` map. **Kartana** = flat unfolded paper (pamphlet, receipt, blank sheet); its reasoning invites the user to fold it into a random real fold from the roster. **Ditto** = no paper at all (a face, a pet, an object, a screen). A *bad* fold is still a fold and never resolves to either. Sentinels are barred from quiz distractors (prompt + server-side), and in the UI they skip the quiz question entirely and render like an error. There is no book pokémon in any generation; Kartana is the only paper-bodied one.
- **Roster trims** (new) — Nacli (box) and Jirachi (yellow star) removed as confusing. Staryu is now the only star; no box fold remains.
